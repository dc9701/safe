...
set TESTPLAN_ROOT=Subject\%PRODUCT%\AutoTests
set TESTLAB_ROOT=Root\%PRODUCT%\AutoTests

set TEST_ROOT=test.cases.lifesaver
...
set RESULT_FILES=%RESULT_PATH%\soapUI\TEST-%TEST_ROOT%.Switchboard.API_Tests.*.xml
set STYLESHEET=%R2QC_RES%\Lifesaver_soapui.xsl

REM echo .
REM echo == Publish the LIFESAVER SOAP UI test results to Quality Center ==
REM echo .
REM echo on
REM perl "%R2QC%" -suite "%SUITE%" -config "%CONFIG%" -product "%PRODUCT%" -release "%RELEASE%" -build "%BUILD%" -qcPlanSubject "%TESTPLAN_ROOT%" -qcLabRoot "%TESTLAB_ROOT%" -testRoot "%TEST_ROOT%" -testGroup "%TEST_GROUP%" -keywords "%KEYWORDS%" -module "%MODULE%" -stylesheet "%STYLESHEET%" -httpProxy "%HTTP_PROXY%" -qcProxy "%QC_PROXY%" %UPDATE_STEPS% -resultFiles "%RESULT_FILES%"
@echo off
...
REM echo .
REM echo == Publish the RUMBLE test results to Quality Center ==
REM echo .
REM echo on
REM perl "%R2QC%" -suite "%SUITE%" -config "%CONFIG%" -product "%PRODUCT%" -release "%RELEASE%" -build "%BUILD%" -qcPlanSubject "%TESTPLAN_ROOT%" -qcLabRoot "%TESTLAB_ROOT%" -testRoot "%TEST_ROOT%" -testGroup "%TEST_GROUP%" -keywords "%KEYWORDS%" -module "%MODULE%" -stylesheet "%STYLESHEET%" -httpProxy "%HTTP_PROXY%" -qcProxy "%QC_PROXY%" %UPDATE_STEPS% -resultFiles "%RESULT_FILES%"
@echo off
