	...
	<test>
		...
		<xsl:choose>
			...
			<xsl:when test="failure">
                <xsl:for-each select="failure">
                    <xsl:choose>
                        <xsl:when test="@message='Not yet implemented'">
		...
		<testSteps>
			<step>
				...
				<xsl:choose>
					<xsl:when test="error">
                        <xsl:attribute name="stepStatus">Failed</xsl:attribute>FAILED - Failure message:  <xsl:value-of select="error/@message"/> <xsl:value-of select="error"/>
						</xsl:when>
