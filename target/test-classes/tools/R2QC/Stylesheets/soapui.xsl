...
<testSuite><xsl:for-each select="testsuite/testcase">
	<test>
		...
		<xsl:attribute name="testScriptPath">{{qcPlanSubject}}.<xsl:value-of select="../@name"/>.<xsl:value-of select="@name"/></xsl:attribute>
		<xsl:attribute name="testSetPath">{{qcLabRoot}}.<xsl:value-of select="../@name"/>.<xsl:value-of select="@name"/></xsl:attribute>
		<xsl:attribute name="testSetNotPassedPath">{{qcLabRootNotPassed}}.<xsl:value-of select="../@name"/>.<xsl:value-of select="@name"/></xsl:attribute>
		<xsl:attribute name="testSetName">.<xsl:value-of select="../@name"/>.<xsl:value-of select="@name"/></xsl:attribute>
		<xsl:choose>
			<xsl:when test="error">
		...
		<testSteps>
			<step>
				...
				<xsl:choose>
					<xsl:when test="error">
						<xsl:attribute name="stepStatus">Not Completed</xsl:attribute>ERROR - <xsl:value-of select="@message"/>
						</xsl:when>
					<xsl:when test="system-err">
						<xsl:attribute name="stepStatus">Failed</xsl:attribute>FAILED - System error:  <xsl:value-of select="."/>
						</xsl:when>
					<xsl:when test="failure">
						<xsl:attribute name="stepStatus">Failed</xsl:attribute>FAILED - Failure message:  <xsl:value-of select="."/><xsl:value-of select="."/>
						</xsl:when>
					<xsl:when test="skipped">
