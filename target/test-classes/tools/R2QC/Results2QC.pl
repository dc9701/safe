...
Results2QC.pl - Publish test results to Quality Center.
...
2. 	For each test case, determine whether a test instance already exists in the appropriate Test Set
	in Quality Center (/Service Manager/AutoTestss/{project_branch}.  If NOT:
...
David C. Cooper (david.c.cooper@hp.com), Service Manager QA, BTO / HP Connected SWS
...
sub AddSteps
{
	...
	# If not adding run steps, remove any extant steps for the test case.

	TestPlanTestStepRemoveAll($TestScriptPath, $TestName) unless $RunId;
	...
	foreach my $Step ($Test->find('./testSteps/step')->get_nodelist) {
		...
		else {							# Add steps of test case in Test Plan if no $RunId.
...
sub AddTestRun
{
	...
	my $RunXml =
		...
		# DCC - HPC only - '<RN_USER_13>'  . $Test->find('./testRun/@runBuild')->string_value()		. '</RN_USER_13>' .
		# DCC - HPC only - '<RN_USER_15>'  . $Test->find('./testRun/@runConfig')->string_value()		. '</RN_USER_15>' .
		# DCC - HPC only - '<RN_USER_16>'  . $Test->find('./testRun/@runEnvironment')->string_value()	. '</RN_USER_16>' .
		'</DATA>';
				...
				if ($TestFolderId = GetTestPlanFolderId($TestScriptPath)) {
					...
					unless ($TestId && ($CurrentPath eq $TestScriptPath)) {
						...
						my $TestXml =
	 						...
	 						# DCC - HPC only - '<TS_USER_36>' 	 	. "100"													. '</TS_USER_36>' .
	 						'<TS_DESCRIPTION>'	. $Test->find('@testDescription')->string_value()		. '</TS_DESCRIPTION>' .
	 						# DCC - HPC only - '<TS_USER_31>' 	 	. $Test->find('@testDuration')->string_value()			. '</TS_USER_31>' .
	 						# DCC - HPC only - '<TS_USER_33>' 	 	. $Test->find('@testKeywords')->string_value()			. '</TS_USER_33>' .
	 						'<TS_USER_23>' 	 	. $Test->find('@testModule')->string_value()			. '</TS_USER_23>' .
	 						# DCC - HPC only - '<TS_USER_32>' 	 	. $Test->find('@testGroup')->string_value()		    	. '</TS_USER_32>' .
		...
		if (exists $InstanceIdCache{$TestSetPath . '\\' . $TestId}) {
			...
			unless ($InstanceId) {
				...
				my $InstanceXml =
					...
					# DCC - HPC only - '<CY_USER_23>'		. "HP Connected"											. '</CY_USER_23>' .
					'<CY_USER_TEMPLATE_01>'		. "$BP_Filter"												. '</CY_USER_TEMPLATE_01>' .
					# DCC - HPC only - '<RN_USER_13>' 		. $Test->find('./testRun/@runBuild')->string_value()		. '</RN_USER_13>' .
					# DCC - HPC only - '<RN_USER_15>' 		. $Test->find('./testRun/@runConfig')->string_value()		. '</RN_USER_15>' .
					# DCC - HPC only - '<RN_USER_16>' 		. $Test->find('./testRun/@runEnvironment')->string_value()	. '</RN_USER_16>' .
					# DCC - HPC only - '<RN_USER_02>'		. "App: HPC"												. '</RN_USER_02>' .
					'</DATA>';
...
sub TestLabRunStepStatusSet
{
	...
	## the question of what to name the steps and what haapens when the names change.
